...
static ssize_t dev_read(struct file *filep, char *buffer, size_t len, loff_t *offset){
   printk("EBBChar: trying to read on offset %d", *offset);
   while (!available) {
   	if (wait_event_interruptible(q, available) != 0) // when interrupted 
	{
		printk("EBBChar: interrupted");
		return 0;
	}
   }
   printk("EBBChar: entering critical section for read which is woken up\n");
   down(&sem);
   available = 0;
   int error_count = 0;
...
static ssize_t dev_write(struct file *filep, const char *buffer, size_t len, loff_t *offset){
   ...
   if (len > 5) {
   	available = 1;
   	printk("EBBChar: Enough characters, waking up\n");
   	wake_up(&q);
   }
...
static unsigned int dev_poll(struct file *filp, poll_table *wait) {
	printk("EBBChar: started polling");
	unsigned int mask = 0;
	while (!available) poll_wait(filp, &q, wait);
	printk("EBBChar: queue awakened");
	if (available) {
		printk("EBBChar: polling is done");
