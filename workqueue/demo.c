...
void count_work(struct work_struct *);
DECLARE_DELAYED_WORK(counter, count_work);
void count_work(struct work_struct * work) {
	time_passed++;
	schedule_delayed_work(&counter, msecs_to_jiffies( 1000 ) );
}
...
static void __exit helloBBB_exit(void){
   cancel_delayed_work_sync(&counter);
   destroy_delayed_work_on_stack(&counter);
...
static ssize_t dev_read(struct file *filep, char *buffer, size_t len, loff_t *offset){
   int error_count = 0;
   sprintf(message, "%ld seconds passed\n", time_passed);
